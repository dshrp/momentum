...
const MomentumApp = () => {
  ...
  const fetchEvents = async (showRefreshIndicator = false) => {
    try {
      ...
      const dylanEvents = (data.events || []).filter((e) => e.title && e.title.toLowerCase().includes("dark ride"))
      console.log(
      ...
      setStats(data.stats || { total: 0, upcoming: 0, past: 0, duplicatesRemoved: 0, userUpdated: 0 })
  ...
  const sources = [
    { name: "Instagram", count: events.filter((e) => e.source === "Instagram").length, status: "connected" },
  ]
  ...
  const eventTypeFilters = Array.from(
    new Set(events.filter((event) => event.source === "Instagram").map((event) => event.type)),
  ).map((type) => ({
    id: type,
    label: type,
    count: events.filter((e) => e.type === type && e.source === "Instagram").length,
    category: "Event Type",
  }))
  ...
  const locationFilters = Array.from(
    ...
    .map((location) => ({
      ...
      label: location,
      count: events.filter((e) => e.location === location).length,
      category: "Location",
    }))
  ...
  // SIMPLIFIED: Filtering and sorting (events are already clean from API)
  ...
  const getExamplePosts = () => {
    try {
      const pastEvents = events.filter((e) => !e.isUpcoming && e.sourceUrl && e.sourceUrl !== "#").slice(0, 3)
      return pastEvents
    } catch (err) {
